...
void Conveyor::InitDefaultCommand() {
    ...
    // BEGIN AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=DEFAULT_COMMAND
        ...
        SetDefaultCommand(new ConveyorDriveWithJoysticks());
...
void Conveyor::ConveyorRunWithJoysticks(std::shared_ptr<frc::Joystick> systemsController)
{
    ...
    if(fabs(leftTrigger) > 0.1)
    {
        output = leftTrigger * modifier;
    }
    else if(fabs(rightTrigger) > 0.1)
    {
        output = rightTrigger * modifier;
    }
    else
    {
        output = 0.0;
